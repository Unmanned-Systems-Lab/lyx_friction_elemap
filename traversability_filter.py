...
def get_filter_torch(*args, **kwargs):
    ...
    class TraversabilityFilter(nn.Module):
        def __init__(self, w1, w2, w3, w_out, device="cuda", use_bias=False):
            ...
            # nn.Conv2d(in_channels,out_channels,kernel_size,stride,padding) dilation：卷积核元素之间的间距 控制卷积核之间的间距
        ...
        def __call__(self, elevation_cupy):
            ...
            # elevation_cupy 
            # elevation_cupy or self.traversability_input ：(self.cell_n, self.cell_n)
            elevation_cupy = elevation_cupy.astype(cp.float32)
            elevation = torch.as_tensor(elevation_cupy, device=self.conv1.weight.device)

            with torch.no_grad():
                # input : (1,1,self.cell_n, self.cell_n)
                # out1 : (1,4,(self.cell_n-2)，(self.cell_n-2))
                # out2 : (1,4,(self.cell_n-4)，(self.cell_n-4))
                # out3 : (1,4,(self.cell_n-6)，(self.cell_n-6))
                ...
                # out1 : (1,4,(self.cell_n-6)，(self.cell_n-6))
                # out2 : (1,4,(self.cell_n-6)，(self.cell_n-6))
                # out3 : (1,4,(self.cell_n-6)，(self.cell_n-6))
                out1 = out1[:, :, 2:-2, 2:-2]
                out2 = out2[:, :, 1:-1, 1:-1]
                # out : (1,12,(self.cell_n-6)，(self.cell_n-6))
                out = torch.cat((out1, out2, out3), dim=1)
                # out = F.concat((out1, out2, out3), axis=1)
                # out : (1,1,(self.cell_n-6)，(self.cell_n-6))
