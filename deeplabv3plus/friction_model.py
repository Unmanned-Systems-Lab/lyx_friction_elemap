...
class FrictionCalculator:
    def __init__(self, yaml_path="/home/lyx/ht_sem_elemap/deeplabv3plus/WildScenes_categories.yaml", num_classes=15):
        """初始化摩擦系数计算器"""
        self.num_classes = num_classes
        # 根据新的15类体系更新忽略索引
        # 原来是 [3, 5, 10, 12, 15, 16]
        # 根据config.py中的CLASSES_15，应该忽略的类别包括:
        # 11 (sky), 6 (other-object), 8 (structure), 9 (other-terrain)，以及可能的14 (water)
        self.ignore_indices = [6, 8, 9, 11, 14]
    ...
    def assign_friction(self, class_probs):
        ...
        """
        # 确保输入的类别概率数量与模型配置的类别数量一致
        if class_probs.shape[0] != self.num_classes:
            raise ValueError(f"类别概率数量({class_probs.shape[0]})与配置的类别数量({self.num_classes})不一致")
